...
interface GetDefaultQueryResolversInput {
  model: VulcanGraphqlModel;
  options: any;
}
export function getDefaultQueryResolvers({
  model,
  options,
}: GetDefaultQueryResolversInput) {
  ...
  return {
    ...
    multi: {
      ...
      async resolver(
        ...
        let { selector, options } = await Connectors.filter(
          collection,
          input,
          context
        );
        ...
        if (canRead) {
          if (typeof canRead === "function") {
            ...
            if (canRead.includes("owners")) {
              ...
              viewableDocs = docs.filter((doc) =>
                Users.isMemberOf(currentUser, canRead, doc)
              );
            } else {
