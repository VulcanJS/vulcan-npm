...
import collectionToGraphQL from "./model";
...
export const GraphQLSchema = {
  ...
  getCollectionsSchemas() {
    const collections = Collections.filter(
      (c) => c.options.generateGraphQLSchema !== false
    );
  ...
  addTypeAndResolvers({ typeName, schema, description = "", interfaces = [] }) {
    ...
    const { fields, resolvers: schemaResolvers = [] } = getSchemaFields(
      schema._schema,
      typeName
    );
    ...
    if ((fields.create || []).length) {
      this.addSchema(
        createDataInputTemplate({ typeName, fields: fields.create })
      );
    }
    // updateTypeDataInput
    if ((fields.update || []).length) {
      this.addSchema(
        updateDataInputTemplate({ typeName, fields: fields.update })
      );
    }
...
export const addStitchedSchema = GraphQLSchema.addStitchedSchema.bind(
  GraphQLSchema
);
export const addTypeAndResolvers = GraphQLSchema.addTypeAndResolvers.bind(
  GraphQLSchema
);
